...
USE SCHEMA l00_stg;
CREATE
OR REPLACE TABLE stg_nation AS
SELECT
    src.*,
    CURRENT_TIMESTAMP() ldts,
    'Static Reference Data' rscr
FROM
    snowflake_sample_data.tpch_sf10.nation src;
CREATE
    OR REPLACE TABLE stg_region AS
SELECT
    src.*,
    CURRENT_TIMESTAMP() ldts,
    'Static Reference Data' rscr
FROM
    snowflake_sample_data.tpch_sf10.region src;
CREATE
    OR REPLACE TABLE stg_customer (
        raw_json VARIANT,
...
--Creating Stream
    CREATE
    OR REPLACE STREAM stg_customer_strm ON TABLE stg_customer;
CREATE
    OR REPLACE STREAM stg_orders_strm ON TABLE stg_orders;
--Creating Internal Stages
    CREATE
    OR REPLACE STAGE customer_data FILE_FORMAT = (TYPE = JSON);
CREATE
    OR REPLACE STAGE orders_data FILE_FORMAT = (TYPE = CSV);
--Copying sample data into stage
...
FROM
    @customer_data;
--Creating Snowpipe to load data from Stage to the Table
    CREATE
    OR REPLACE PIPE stg_orders_pp AS COPY INTO stg_orders
FROM
    (
        SELECT
            $1,
...
CREATE
    OR REPLACE PIPE stg_customer_pp --AUTO_INGEST = TRUE
    --aws_sns_topic = 'arn:aws:sns:mybucketdetails'
    AS COPY INTO stg_customer
FROM
    (
        SELECT
            $1,
...
- Manually triggering the data
from
    the specified Snowpipe ALTER PIPE stg_customer_pp REFRESH;
...
SELECT
    ...
    count(1)
FROM
    stg_customer_strm CREATE
    OR REPLACE VIEW stg_customer_strm_outbound AS
SELECT
    src.*,
...
FROM
    stg_customer_strm src;
CREATE
    OR REPLACE VIEW stg_order_strm_outbound AS
SELECT
    src.*
    --------------------------------------------------------------------
    -- derived business key
